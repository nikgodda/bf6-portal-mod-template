...
Use this template to build any structured BF6 Portal mod using classes, folders, and TypeScript modules — the framework turns it into one final script.
...
│   package.json
│   tsconfig.json
│   __SCRIPT.ts         ← generated by build
│   __STRINGS.json      ← generated by build
│
├─ SDK/
│   ├─ mod/             ← official Battlefield Portal typings
│   └─ modlib/
│
└─ src/
    ├─ main.ts          ← entry point (creates game mode)
    ├─ Core/
    │    └─ AGameMode.ts
    └─ GameModes/
         (your modes here)
...
> ⚠ `__STRINGS.json` is **not** updated in watch mode.  
...
Everything else is normal TypeScript — classes, interfaces, modules, nested files — the framework will merge it into one script.
...
→ `"static.messageWithParams": "static.messageWithParams {}"`
...
→ `"static.stringkey": "static.stringkey"`
...
# 🔥 Full Real Example from Template
...
-   `__SCRIPT.ts` → paste into Portal Web Editor
